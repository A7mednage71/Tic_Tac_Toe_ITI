...
public class BoardController implements Initializable {
    ...
    @FXML private StackPane drawOverlay;
    ...
    private void makeAiMove() {
        switch (GameSession.difficulty) {
            case EASY:
                ...
                break;
            case MEDIUM:
                if (!smartMove("X")) easyMove();
                break;
            case HARD:
                if (smartMove("O")) return;
                if (smartMove("X")) return;
    ...
    private boolean smartMove(String symbol) {
        ...
        for (int i = 0; i < 3; i++) {
            if (findEmptyInLine(grid[i][0], grid[i][1], grid[i][2], symbol)) return true;
            if (findEmptyInLine(grid[0][i], grid[1][i], grid[2][i], symbol)) return true;
        }
        if (findEmptyInLine(grid[0][0], grid[1][1], grid[2][2], symbol)) return true;
        if (findEmptyInLine(grid[0][2], grid[1][1], grid[2][0], symbol)) return true;
        return false;
    }

    private boolean findEmptyInLine(Button b1, Button b2, Button b3, String symbol) {
        if (b1.getText().equals(symbol) && b2.getText().equals(symbol) && b3.getText().isEmpty()) { playMove(b3, "O"); return true; }
        if (b1.getText().equals(symbol) && b3.getText().equals(symbol) && b2.getText().isEmpty()) { playMove(b2, "O"); return true; }
        if (b2.getText().equals(symbol) && b3.getText().equals(symbol) && b1.getText().isEmpty()) { playMove(b1, "O"); return true; }
        return false;
    }
    ...
    private void finishGame(String message) {
        ...
        updateScoreLabels();
        Platform.runLater(() -> {
            if (message.contains("X Wins")) playVideo("/videos/winner.mp4");
            else if (message.contains("O Wins")) {
                if (GameSession.vsComputer) playVideo("/videos/loser.mp4");
                else playVideo("/videos/winner.mp4");
            } else {
                showDrawAlert();
            }
        });
    ...
    private void highlightWinningButtons(Button... buttons) {
        for (Button btn : buttons) btn.getStyleClass().add("winning-button");
    }
    ...
    private void handlePlayAgain(ActionEvent event) {
        for (Node node : gameGrid.getChildren()) {
            if (node instanceof Button) {
                ...
                if (!btn.getStyleClass().contains("cell-button-empty")) btn.getStyleClass().add("cell-button-empty");
            }
        }
        gameOver = false;
        isXTurn = true;
        if (!GameSession.vsComputer) opponentNameLabel.setText("Player 1's Turn (X)");
    }

    @FXML
    private void handleBack(ActionEvent event) {
        try { switchScene(GameSession.vsComputer ? "vsComputer.fxml" : "Home.fxml", event); } catch (Exception ex) { ex.printStackTrace(); }
    }
